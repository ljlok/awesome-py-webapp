...
class HttpError(Exception):
    '''
    HttpError defines http error code.
...
class RedirectError(HttpError):
    '''
    RedirectError that defined http redirect code.
...
class Request(object):
    ...
    def environ(self):
        '''
        Get raw environ dict, both key, value are str.
    ...
    def host(self):
        '''
        Get requst host as str. Default to '' if cannot get.
...
class Response(object):
    ...
    def unset_header(self, name):
        ...
        key = name.upper()
        if key not in _RESPONSE_HEADER_DICT:  # not in instead of not xx in
            key = name
    ...
    def get_wsgi_application(self, debug=False):
        ...
        def wsgi(env, start_response):
            ...
            try:
                ...
                fp.close()
                start_response('500 Internal Server Error', [])
                return [r'''<html><body><h1>500 Internal Server Error</h1><div style="font-family:Monaco, Menlo, Consolas, 'Courier New', monospace;"><pre>''', stacks.replace('<', '&lt;').replace('>', '&gt;'), '</pre></div></body></html>']
            finally:
                del ctx.application
                del ctx.request
                del ctx.response
